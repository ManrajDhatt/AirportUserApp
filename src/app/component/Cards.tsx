...
//   const isInCart = cartItems.some((item:Product) => item.id === currentProduct.id);
//   const isOutOfStock = parseInt(currentProduct.stock, 10) === 0;
//   const hasDiscount = currentProduct.discount && currentProduct.finalPrice;
...
//   if (!currentProduct.id || !currentProduct.catalogueCategoryId) {
//     console.warn(`Invalid product data for ${currentProduct.catalogueProductName}:`, {
//       id: currentProduct.id,
//       catalogueCategoryId: currentProduct.catalogueCategoryId,
//     });
//     return; // Skip adding to cart
//   }

//   if (isInCart) {
//     setCartItems((prev: Product[]) => prev.filter((item) => item.id !== currentProduct.id));
//     await removeFromCart(currentProduct.id);
//   } else {
//     const newItem = {
//       id: currentProduct.id,
...
//       CategoryId: currentProduct.catalogueCategoryId,
//     };
//     setCartItems((prev: Product[]) => [...prev, newItem]);
//     await addToCart(newItem, storeId);
//   }
// };
//   // const handleCartToggle = async () => {
...
// Define CartItem interface to match StoreContext
interface CartItem {
  id: string;
  name: string;
  price: number;
...
const Cards: React.FC<CardProps> = ({ product: initialProduct, storeId }) => {
  ...
  const isInCart = cartItems.some((item: CartItem) => item.id === currentProduct.id);
  const isOutOfStock = parseInt(currentProduct.stock, 10) === 0;
  const hasDiscount = currentProduct.discount && currentProduct.finalPrice;
  ...
  const handleCartToggle = async () => {
    ...
    if (isInCart) {
      setCartItems((prev: CartItem[]) => prev.filter((item) => item.id !== currentProduct.id));
      await removeFromCart(currentProduct.id);
    } else {
