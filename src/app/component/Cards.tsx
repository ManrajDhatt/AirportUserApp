...
// 	const handleCartToggle = async () => {
// 		if (isInCart) {
// 		  setCartItems((prev: CartItem[]) => prev.filter((item) => item.id !== product.id));
// 		  await removeFromCart(product.id);
// 		} else {
// 		  const newItem: CartItem = {
// 			id: product.id,
...
// 			CategoryName: "", // You might need to add this to your Product interface
  ...
  const handleCartToggle = async () => {
    if (isInCart) {
      ...
      const newItem = {
        ...
        productImageUrl: product.productImageUrl || "/default-image.jpg",
        productDescription: product.productDescription || "A great product for you!",
        CategoryName: "", // Add this if needed by your context
        CategoryId: product.catalogueCategoryId
      };
      setCartItems(prev => [...prev, newItem]);
      await addToCart(newItem, storeId);
