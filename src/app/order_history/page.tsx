...
type ProductStatus = "pending" | "accepted" |"reviewed" | "rejected" | "picked" | "delivered";
...
const OrderHistoryPage = () => {
  ...
  const markOrderAsDelivered = async (orderId: string) => {
    try {
      ...
      // Also update all store orders to delivered
      const storeOrdersRef = collection(db, `Orders/${orderId}/StoreOrders`);
      const storeOrdersSnapshot = await getDocs(storeOrdersRef);
      
      const updatePromises = storeOrdersSnapshot.docs.map(async (storeOrderDoc) => {
        const storeOrderRef = doc(db, `Orders/${orderId}/StoreOrders`, storeOrderDoc.id);
        ...
        return new Promise<Order>((resolve) => {
          const unsubscribeStoreOrders = onSnapshot(storeOrdersQuery, async (storeOrdersSnapshot) => {
            ...
            const order: Order = {
              ...
              status: orderData.status==="completed"?"delivered":orderData.status,
  ...
  const getStatusColor = (status: OrderStatus) => {
    switch(status) {
      ...
      case "assigned": return "bg-indigo-100 text-indigo-800";
      case "picked": return "bg-purple-100 text-purple-800";
      // case "packaged": return "bg-orange-100 text-orange-800";
      case "delivered": return "bg-emerald-100 text-emerald-800";
      default: return "bg-gray-100 text-gray-800";
    }
  };
    ...
    <div className="mt-24 container mx-auto p-4 min-h-screen">
      ...
      <div className="mb-8">
        <div className="flex justify-center flex-wrap gap-2">
          {["All", "pending", "accepted", "rejected","reviewed", "assigned","picked", "delivered"].map((status, index) => (
            <motion.button
              key={status}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
